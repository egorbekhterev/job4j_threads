...
public class Examples {
    ...
    /** Первый пример. Отец работает. Сын выбрасывает мусор асинхронно.*/
    ...
    /** CompletableFuture<Void>.runAsync() - выполняет асинхронное действие, не возвращая результата.*/
    private static CompletableFuture<Void> goToTrash() {
        return CompletableFuture.runAsync(() -> {
            System.out.println("Сын: Пап, я пошел выносить мусор");
            try {
    ...
    /** Запуск основного потока и асинхронной задачи.*/
    public static void runAsyncExample() throws Exception {
        CompletableFuture<Void> gtt = goToTrash();
        iWork();
    }

    /** Второй пример. CompletableFuture<String>.supplyAsync() - возвращает результат асинхронного действия.
     * Сын пошел за молоком в магазин асинхронно.*/
    public static CompletableFuture<String> buyProduct(String product) {
        return CompletableFuture.supplyAsync(() -> {
            System.out.println("Сын: Пап, я пошел в магазин");
            try {
                TimeUnit.SECONDS.sleep(5);
            } catch (InterruptedException e) {
                e.printStackTrace();
    ...
    /** Запуск основного потока и асинхронной задачи для второго примера.*/
    public static void supplyAsyncExample() throws Exception {
        CompletableFuture<String> buyMilk = buyProduct("молоко");
        iWork();
        System.out.println("Куплено: " + buyMilk.get());
    }

    /** thenRun() - выполняет задачу типа Runnable после выполнения асинхронной задачи.*/
    ...
    /** thenAccept() - выполняет не задачу целиком, а конкретное действие по окончанию асинхронной операции.
     * Принимает Consumer.*/
    ...
    /** thenApply() - принимает Function. Результат преобразования доступен после вызова get().*/
    ...
    /** thenCompose() - принимает Function. Организует последовательность выполнения асинхронных действий.*/
    ...
    /** thenCombine() - принимает второе действие в качестве первого аргумента, а BiFunction
     * принимает в качестве второго. Используются для независимого выполнения действий.
     * В BiFunction результат двух задач преобразуем в общий результат.*/
    ...
    /** Создаем асинхронную задачу для мытья рук.*/
    ...
    /** Обеспечивает выполнение всех асинхронных задач.*/
    ...
    /** Результат - первая выполненная асинхронная задача.
     * Метод get() - блокирующий. */
    ...
    /** Вычисление суммы элементов диагоналей матрицы.*/
